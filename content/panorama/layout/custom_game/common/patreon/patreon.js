...
class PatreonPerk {
	constructor(name, level, overrideImage) {
		...
		this.panel
			.FindChildTraverse("PatreonPerkImage")
			.SetImage(overrideImage || "file://{resources}/layout/custom_game/common/patreon/perks/" + name + ".png");
...
function UpdatePaymentTargetList(patreonData) {
	if (donation_target_dropdown) {
		...
		layout_string += `<Label text="${Players.GetPlayerName(
			local_id,
		)}" id="PatreonOption${local_id}" onmouseover="UpdatePaymentTarget(${local_id})" />`;
		...
		for (var id = 0; id <= 23; id++) {
			if (Players.IsValidPlayerID(id) && id != local_id) {
				if (!patreonData[id] || patreonData[id].level <= 0) {
					layout_string += `<Label text="${Players.GetPlayerName(
						id,
					)}" id="PatreonOption${id}" onmouseover="UpdatePaymentTarget(${id})" />`;
				}
			}
		}
