...
function Timers:Think()
    ...
    for _, cb in ipairs(nextTickCallbacks) do ErrorTracking.Try(cb) end
    ...
    for k, v in pairs(Timers.timers) do
        ...
        if now >= v.endTime then
            ...
            -- Run the callback
            local status, nextCall = ErrorTracking.Try(v.callback,
                                                       GameRules:GetGameModeEntity(),
                                                       v)

            table.insert(profileFunctions,
                         {v.callback, GetSystemTimeMS() - funcTime})
    ...
    if profileTime > Timers._timeLimit and nFuncs > 0 then
        ...
        for _, funcData in pairs(profileFunctions) do
            ...
            if funcObj ~= nil then
				if funcTime >= PERF_REPORT_THRESHOLD then -- single callback used more then 0.5ms of server frame time, reported
					local funcInfo = debug.getinfo(funcObj)
					local traceback_line = funcInfo.short_src .. ":" .. funcInfo.linedefined
					print("[Warning.Timers] Function", traceback_line, "took", funcTime, "ms")
					Timers._badPerformanceTimers[traceback_line] = ((Timers._badPerformanceTimers[traceback_line] or funcTime) + funcTime) / 2
				end
            end
        end
...
function Timers:RemoveTimer(name)
    if Timers.timers[name] then Timers.timers[name] = nil end
end
